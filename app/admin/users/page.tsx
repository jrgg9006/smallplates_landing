...
export default function AdminUsersPage() {
  ...
  const handleDelete = async (user: User) => {
    if (!confirm(`Are you sure you want to delete user ${user.email}?\n\nThis will permanently delete the user and ALL their data (recipes, guests, groups, etc.). This action cannot be undone.`)) {
      return;
    }
  ...
  const handleCleanConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/users/${userToClean.id}/clean`, {
        method: 'POST',
      });
  ...
  const handleResetOnboarding = async (user: User) => {
    ...
    try {
      const response = await fetch(`/api/admin/users/${user.id}/reset-onboarding`, {
        method: 'POST',
      });
