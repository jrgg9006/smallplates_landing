...
import { uploadGroupDashboardImageWithClient, deleteGroupDashboardImage } from '@/lib/supabase/storage';

/**
 * POST /api/v1/groups/[groupId]/dashboard-image
 * Upload or update dashboard image for a group
 */
export async function POST(
  ...
  try {
    ...
    console.log('Starting dashboard image upload for groupId:', groupId);
    ...
    // Allow any group member (owner, admin, captain, member) to upload dashboard image
    ...
    const { url, error: uploadError } = await uploadGroupDashboardImageWithClient(
      supabase,
      groupId,
      file
    );
    ...
    if (updateError) {
      ...
      await deleteGroupDashboardImage(groupId);
      return NextResponse.json(
...
/**
 * DELETE /api/v1/groups/[groupId]/dashboard-image
...
export async function DELETE(
  ...
  try {
    ...
    if (memberError || !membership) {
      return NextResponse.json(
        { error: 'Permission denied. You must be a group member to delete the dashboard image.' },
        { status: 403 }
      );
    }
    
    // Allow any group member (owner, admin, captain, member) to delete dashboard image

    // Delete image from storage
    const { error: deleteStorageError } = await deleteGroupDashboardImage(groupId);
