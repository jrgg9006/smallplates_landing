...
// Reason: founder tip promoting the Send Reminders tool. Goes to the organizer
// (owner) of an active book. No hard cap — the dashboard shows the trail so the
// founder decides. Sending to someone who already uses the tool is allowed but
// pointless; the dashboard flags it.
export async function POST(request: NextRequest) {
  try {
    ...
    const subject = buildRemindersTipSubject();
    ...
    if (logError) {
      console.error('Failed to log reminders_tip email:', logError.message);
    }
