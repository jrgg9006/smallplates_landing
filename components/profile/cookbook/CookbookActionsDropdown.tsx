...
interface CookbookActionsDropdownProps {
  cookbook: Cookbook;
...
export function CookbookActionsDropdown({
  cookbook,
  onDeleteCookbook,
  onExitCookbook,
  className,
}: CookbookActionsDropdownProps) {
          ...
          >
            {isGroupCookbook ? (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  handleAction(onExitCookbook);
                }}
              >
                <LogOut className="h-4 w-4" />
                Exit Shared Cookbook
              </button>
            ) : (
              <button
                className="w-full px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-2"
