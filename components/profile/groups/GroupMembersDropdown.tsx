...
export function GroupMembersDropdown({ group, onInviteFriend }: GroupMembersDropdownProps) {
  ...
  const loadPendingInvitations = async () => {
    try {
      ...
      const { data, error } = await getGroupPendingInvitations(group.id);
    ...
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className="w-full md:w-auto h-12 md:h-auto flex items-center justify-center gap-2 px-3 md:py-2 border border-gray-300 rounded-lg bg-white text-base md:text-sm font-medium text-gray-700 hover:bg-gray-50" disabled={loading}>
            ...
            <>
              ...
              {loadingInvitations ? (
                ...
                pendingInvitations.map((invitation) => {
                  // Parse name - split by space for first/last name
