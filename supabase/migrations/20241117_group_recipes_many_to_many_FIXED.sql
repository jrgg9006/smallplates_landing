...
-- Step 6: CLEAR the old group_id values BEFORE adding the constraint
UPDATE public.guest_recipes 
SET group_id = NULL 
WHERE group_id IS NOT NULL;

-- Step 7: NOW add the constraint to prevent future direct group assignments
...
-- Step 8: Create a view to simplify querying recipes with their groups
...
-- Step 9: Create helper function to get all groups a recipe belongs to
...
-- Step 10: Create function to check if a recipe is in a specific group
...
-- Migration complete!
-- Your recipes can now belong to multiple groups simultaneously!
-- All existing group associations have been preserved in the new structure.