...
function copyTitleToFooter(spread) {
    for (var p = 0; p < spread.pages.length; p++) {
        ...
        for (var i = 0; i < items.length; i++) {
            if (items[i].label === "recipeTitle") titleText = items[i].contents;
            if (items[i].label === "recipeFooter") footerFrame = items[i];
        }
        
        if (footerFrame && titleText) {
            footerFrame.contents = stripEmojis(titleText);
        }
    }
}
