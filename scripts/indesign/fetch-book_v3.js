...
async function fetchBook() {
  ...
  for (const recipe of recipes) {
    ...
    // Firma del invitado (opcional, PNG transparente). Reason: solo se agrega la clave
    // signature_image_path cuando la receta tiene firma, para que el JSON sea idéntico al
    // viejo en recetas/libros sin firma (cero-regresión). Descarga tolerante a fallos: si
    // falla, se omite la firma y la receta sigue igual.
    if (recipe.signature_url) {
      try {
        ...
        process.stdout.write(`   ✍️  firma ${recipe.recipe_name}... `);
        await downloadImage(recipe.signature_url, signatureDest);
  ...
  if (signaturesDownloaded > 0 || signaturesFailed > 0) {
    console.log(`  ✍️  Firmas: ${signaturesDownloaded}${signaturesFailed > 0 ? `, ${signaturesFailed} fallidas` : ''}`);
  }
  console.log('');
