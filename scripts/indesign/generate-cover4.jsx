// ============================================
// SMALL PLATES — Cover Generator v4.0
...
//
// USO: Abrir el template master → Scripts panel → generate-cover4.jsx
//
// CAMBIOS vs v3.0:
//  4. La portada usa el BOOK NAME (couple.couple_display_name del JSON) como única
//     fuente de verdad, partiéndolo en "&" hacia NAME_1/NAME_2. Si el book name no
//     trae "&" (un solo título, ej. "Richard's Birthday"), limpia el ampersand y
//     NAME_2 y centra NAME_1. name2 ya NO es obligatorio.
//     ⚠️ VERIFICAR EN INDESIGN: el caso de nombre único (centrado + back subtitle
//        con placeholder "<<name_1>> & <<name_2>>" que puede dejar un "&" suelto).
//
// CAMBIOS vs v1.0:
...
//
// CAMBIOS vs v2.0:
//  3. Lee couple.cover_line del JSON del libro y sobreescribe el eyebrow del front
//     (COVER_SUBHEAD) cuando se proporciona. Permite personalización desde el dashboard.
//
...
function main() {
    ...
    if (input.jsonPath && input.jsonPath.length > 0) {
        ...
        if (bookData) {
            // Reason: el Book Name es la única fuente de verdad. Usamos
            // couple_display_name (el nombre del libro), partiéndolo en "&" hacia los
            // dos frames de nombre. Solo si no hay book name caemos a los nombres
            // individuales (compatibilidad con datos viejos).
            var bookName = getNestedValue(bookData, 'couple.couple_display_name');
            if (bookName) {
                var ampIdx = bookName.indexOf('&');
                if (ampIdx > -1) {
                    name1 = bookName.substring(0, ampIdx).replace(/(^\s+|\s+$)/g, '');
                    name2 = bookName.substring(ampIdx + 1).replace(/(^\s+|\s+$)/g, '');
                } else {
                    name1 = bookName.replace(/(^\s+|\s+$)/g, '');
                    name2 = '';
                }
            } else {
            ...
            coverLine = getNestedValue(bookData, 'couple.cover_line') || coverLine;
        }
    }

    // Validación — name2 es opcional (un libro puede tener un solo nombre/título)
    if (!name1 || name1 === "Nombre") {
        alert("Error: Ingresa el nombre del libro.");
        return;
    }
    ...
    var outName = "Cover_" + sanitizeFileName(name1) + (name2 ? "-" + sanitizeFileName(name2) : "") + ".indd";
    var newFile = File(parentFolder.fsName + "/" + outName);

    // Log
    var summary = "Small Plates — Cover Generator v4.0\n\n" +
                  "Nombre del libro: " + (name2 ? name1 + " & " + name2 : name1) + "\n" +
                  "Páginas: " + pageCount + "\n" +
    ...
    // --- Paso 7c: Layout de NOMBRE ÚNICO (Book Name sin "&") ---
    // Reason: cuando el Book Name no trae "&" (ej. "Richard's Birthday") no hay dos
    // nombres. Limpiamos el ampersand y el NAME_2 (para que no quede un "&" suelto)
    // y centramos NAME_1 en el front. OJO: este caso requiere verificación visual
    // en InDesign — el template está armado para "A & B".
    if (!name2) {
        var ampSingle = findByLabel(doc, LABELS.AMPERSAND);
        if (ampSingle) ampSingle.contents = "";
        var name2Single = findByLabel(doc, LABELS.NAME_2);
        if (name2Single) name2Single.contents = "";
        var name1Single = findByLabel(doc, LABELS.NAME_1);
        if (name1Single) {
            var fullW = BASE.NAME_1_W + BASE.AMPERSAND_W + BASE.NAME_2_W;
            var leftX = pos.frontCenter - fullW / 2;
            name1Single.geometricBounds = [
                BASE.NAMES_Y,
                leftX,
                BASE.NAMES_Y + BASE.NAME_1_H,
                leftX + fullW
            ];
            try {
                name1Single.parentStory.paragraphs.everyItem().justification = Justification.CENTER_ALIGN;
            } catch (e) {}
        }
    }

    // --- Paso 7b: Sobreescribir eyebrow del front cuando viene del JSON ---
    // Reason: el eyebrow es ahora editable por el owner desde el dashboard;
    // solo se sobreescribe cuando se proporciona un valor, para no romper
    // el texto por defecto del template cuando se usa el script manualmente.
    if (coverLine) {
        var subheadFront = findByLabel(doc, LABELS.SUBHEAD);
        if (subheadFront) {
            subheadFront.contents = String(coverLine).toUpperCase();
        }
    }
