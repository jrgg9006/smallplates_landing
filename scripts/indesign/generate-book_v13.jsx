// ============================================
// SMALL PLATES — Book Generator v1.11.0
//
...
function main() {
    ...
    if (!confirm("SMALL PLATES - Book Generator v1.11.0\n\n" +
    ...
    // ── STEP 11.5: Remove INDEX_OVERFLOW pages if empty ──
    // Reason: The master template has 2 extra TOC pages (INDEX_OVERFLOW) for books
    // with 49+ recipes. After the TOC is populated, if these pages have no real text
    // content, they are removed to avoid blank pages in the final book.
    $.writeln("\n=== Checking INDEX_OVERFLOW ===");
    var indexOverflow = findSpreadByLabel(doc, CONFIG.labels.indexOverflow);

    if (indexOverflow) {
        var hasContent = false;
        for (var iop = 0; iop < indexOverflow.pages.length && !hasContent; iop++) {
            var ioItems = indexOverflow.pages[iop].allPageItems;
        ...
        if (!hasContent) {
            try {
                removeSpread(indexOverflow);
                stats.indexOverflowRemoved = true;
        ...
        $.writeln("INDEX_OVERFLOW not found, skipping");
    }
    ...
    // ── REPORT ──
    var msg = "LIBRO GENERADO (v1.11.0)\n\n";
    ...
    msg += "\nIndex overflow: " + (stats.indexOverflowRemoved ? "Removed (not needed)" : "Kept (has content)");
