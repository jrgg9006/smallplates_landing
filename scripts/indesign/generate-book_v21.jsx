// ============================================
// SMALL PLATES — Book Generator v1.19.0 (file v21)
//
// v1.19.0 (v21): Optional guest signature. Reads recipe.signature_image_path and places
//         the transparent PNG into a {{SIGNATURE}} frame (whole image, centered, on top).
//         Fully optional + zero-error: no-op when the recipe has no signature, the file is
//         missing, or the template lacks the {{SIGNATURE}} frame (masters without it).
//         Nothing else changes vs v20 — pagination, spread moves, Originals, allowPageShuffle
//         are all untouched. The {{SIGNATURE}} frame is added to each recipe template in Phase 2.
//
...
// v1.16.0: ORIGINALS rebuilt as PAGE templates (index-style auto-fill), replacing
//          the v1.15.0 opener/pair SPREAD model. Master carries 3 template pages
//          (removed after generation, always — even for no-annex books):
//            · originalsTitle page ("The Originals" opener, static copy)
//            · 1-up page: {{ORIGINAL_IMAGE}} + originalCaption
//            · 2-up page: {{ORIGINAL_IMAGE_TOP}} + {{ORIGINAL_IMAGE_BOTTOM}} + originalCaption
//          Page plan: images grouped per recipe (position order), chunked in
//          pairs -> 2 images of one recipe share a 2-up page with ONE caption;
//          a leftover single gets a 1-up page. Pages are INSERTED right after
//          the OBJECTS_THAT_STAY spread (title first), plus one blanked filler
//          page when the inserted count would be odd (keeps recto/verso parity
//          for everything after the section). Cross-ref footers unchanged.
//          ZERO-REGRESSION: no annex_images -> nothing inserted, templates removed.
// v1.17.0: FIX critical — v1.16.0 toggled allowPageShuffle to insert loose pages
//          mid-document, which rebalanced EVERY page across spreads (section
//          landed after the first recipe, frames shifted sides ~1cm, cross-ref
//          footers written into the wrong recipes). Rebuilt with the same safe
//          primitives recipes use: duplicate whole SPREADS at end + spread.move
//          into place. The [title|1up] template spread doubles as the shell for
//          every section spread; slots that need a different layout are cleared
//          and rebuilt by copying the template page's items (position preserved
//          relative to the page). Section is moved AFTER the spread that carries
//          an item labeled "originalsAnchor" (last page of Objects-that-stay) —
//          the OBJECTS_THAT_STAY spread label marks the START of that section
//          (recipes move before it), not its end. Report: QR lines trimmed to
//          Placed/None (no paths/URLs).
// v1.18.0: Three fixes from the first v1.17.0 run (Emily & John):
//          · copyPageItemsTo MIRRORS X when source and target pages sit on
//            opposite spread sides — the master uses mirrored (inner/outer)
//            margins, so a right-page layout copied verbatim onto a left page
//            landed ~1cm off. Centered items are unaffected by the mirror.
//          · Every generated content/blank page gets the PARENT of the 1-up
//            template page applied, so running footers ("Small Plates &
//            Company" left / "Still at the Table" right) and folios come from
//            the parent, correct per side. The template pages must NOT carry
//            local footer/folio frames (they'd be copied verbatim to either
//            side — that's how "Still at the Table" showed up on left pages).
//          · STEP 10.5 removes the buffer blank by the REFERENCE captured in
//            STEP 2, never by "last page looks blank": the last printed page
//            (orange, a plain color rectangle with no text/images) passed the
//            blank heuristic and was deleted. If the buffer already died with
//            the [2up|blank] template spread in STEP 10, nothing is removed.
// ============================================
...
var CONFIG = {
    ...
    // v21: firma manuscrita del invitado (opcional). El marco {{SIGNATURE}} se dibuja
    // en cada template de receta (Fase 2). Si no existe el marco o la receta no tiene
    // firma, el placement es no-op silencioso.
    signatureImageLabel: "{{SIGNATURE}}",

    // Originals section (v1.16.0). Template PAGES are located by these item
    // labels (no spread labels): the title page carries originalsTitle, the
    // 1-up page carries {{ORIGINAL_IMAGE}}, the 2-up page carries the TOP frame.
    originalImageLabel: "{{ORIGINAL_IMAGE}}",
    originalImageTopLabel: "{{ORIGINAL_IMAGE_TOP}}",
    originalImageBottomLabel: "{{ORIGINAL_IMAGE_BOTTOM}}",
    originalCaptionLabel: "originalCaption",
    originalNoteFooterLabel: "originalNoteFooter",
    originalsTitleLabel: "originalsTitle",
    // Any frame on the LAST page of the Objects-that-stay section carries this
    // label; the Originals section is moved right AFTER that spread. (The
    // OBJECTS_THAT_STAY spread label can't be used: it marks the FIRST spread
    // of the section — recipes are moved before it in STEP 9.)
    originalsAnchorLabel: "originalsAnchor",
...
function processRecipeOnTemplate(templateSpread, savedPositions, recipe, basePath) {
    ...
    placeSignatureInSpread(newSpread, recipe, basePath);
...
function main() {
    ...
    // ── ORIGINALS gate + page plan (v1.16.0) ──
    // Reason: build the exact list of pages the section needs, in book order.
    // Per recipe (images sorted by position) chunk in pairs: 2 images -> one
    // 2-up page sharing a single caption; a leftover single -> one 1-up page.
    // (3 images = 2-up + 1-up, 4 = two 2-up, etc.) If the plan is empty the
    // whole feature is inert and the run matches v16 exactly.
    var annexPagePlan = [];
    var annexImageCount = 0;
    for (var ax = 0; ax < recipes.length; ax++) {
        ...
        if (rcAx.annex_images && rcAx.annex_images.length > 0) {
            ...
            for (var im = 0; im < imgsAx.length; im++) {
                if (imgsAx[im].local_image_path) pathsAx.push(imgsAx[im].local_image_path);
            }
            for (var ck = 0; ck < pathsAx.length; ck += 2) {
                annexPagePlan.push({
                    recipeId: rcAx.id,
                    recipe: rcAx,
                    caption: buildOriginalCaption(rcAx),
                    images: pathsAx.slice(ck, ck + 2)
                });
            }
            annexImageCount += pathsAx.length;
        }
    }
    var hasAnnex = annexPagePlan.length > 0;
    ...
    if (!confirm("SMALL PLATES - Book Generator v1.18.0\n\n" +
                 ...
                 "Originals: " + (hasAnnex ? annexImageCount + " imagen(es) en " + annexPagePlan.length + " pagina(s)" : "ninguno") + "\n\n" +
    ...
    // Originals template PAGES (v1.16.0). Looked up ALWAYS (not gated on
    // hasAnnex): even a no-annex book must find them so STEP 10 removes them.
    // Each page is located by an item label it carries; references are captured
    // BEFORE any duplication so later copies of the same labels can't confuse us.
    var originalsTitleTplPage = findTemplatePageByItemLabel(doc, CONFIG.originalsTitleLabel);
    var originals1upTplPage = findTemplatePageByItemLabel(doc, CONFIG.originalImageLabel);
    var originals2upTplPage = findTemplatePageByItemLabel(doc, CONFIG.originalImageTopLabel);

    // Anchor spread for the Originals section (v1.17.0): the spread holding the
    // item labeled originalsAnchor (last page of Objects-that-stay). Captured
    // now, before any duplication, like the template pages.
    var originalsAnchorSpread = null;
    try {
        var originalsAnchorPage = findTemplatePageByItemLabel(doc, CONFIG.originalsAnchorLabel);
        if (originalsAnchorPage) originalsAnchorSpread = originalsAnchorPage.parent;
    } catch (eAnchor) {}

    // Parent for generated Originals pages (v1.18.0): the one applied to the
    // 1-up template page. Parents render footers/folios per page side, so the
    // same parent is correct on both left and right generated pages.
    var originalsContentParent = null;
    try {
        if (originals1upTplPage) originalsContentParent = originals1upTplPage.appliedMaster;
    } catch (eParent) {}

    // Buffer blank page captured by REFERENCE (v1.18.0), before any duplication.
    // STEP 10.5 removes exactly this page (if still alive) — never "the last
    // page if it looks blank": the orange colophon is a plain color rectangle
    // and passes any text/image blank heuristic.
    ...
    // ── STEP 9b: Generate ORIGINALS section (v1.17.0) ──
    // Reason: the section is built with the SAME primitives recipes use —
    // duplicate whole spreads AT_END, fill them, then spread.move() into place
    // right after the originalsAnchor spread (last page of Objects-that-stay).
    // No loose-page insertion, no allowPageShuffle — v1.16.0 toggled shuffle and
    // InDesign rebalanced every page in the document. Runs AFTER STEP 9 so the
    // anchor region is final. Cross-ref page numbers are filled later (STEP 11.7).
    var originalsStatus = "Not attempted";
    var originalsSpreadsArr = [];
    if (hasAnnex) {
        var needs2up = false;
        for (var n2 = 0; n2 < annexPagePlan.length; n2++) {
            if (annexPagePlan[n2].images.length === 2) { needs2up = true; break; }
        }
        var missingTpl = [];
        if (!originalsTitleTplPage) missingTpl.push(CONFIG.originalsTitleLabel + " (pagina titulo)");
        if (!originals1upTplPage) missingTpl.push(CONFIG.originalImageLabel + " (pagina 1-up)");
        if (needs2up && !originals2upTplPage) missingTpl.push(CONFIG.originalImageTopLabel + " (pagina 2-up)");
        if (!originalsAnchorSpread) missingTpl.push(CONFIG.originalsAnchorLabel + " (frame en la ULTIMA pagina de Objects that stay)");

        // The [title|1up] spread doubles as the shell for every section spread:
        // title must sit on the LEFT page and the 1-up on the RIGHT of ONE spread.
        if (originalsTitleTplPage && originals1upTplPage) {
            var openerOk = false;
            try {
                openerOk = originalsTitleTplPage.parent.id === originals1upTplPage.parent.id &&
                    originalsTitleTplPage.parent.pages[0].id === originalsTitleTplPage.id;
            } catch (eOp) {}
            if (!openerOk) missingTpl.push("portada y 1-up deben compartir UN spread (portada izquierda, 1-up derecha)");
        }

        if (missingTpl.length === 0) {
            try {
                generateOriginalsSpreads(originalsTitleTplPage, originals1upTplPage, originals2upTplPage, annexPagePlan, basePath, originalsAnchorSpread, originalsSpreadsArr, recipeFirstOrigPage, originalsContentParent);
                originalsStatus = "Generated " + annexImageCount + " image(s) on " + annexPagePlan.length + " page(s), " + originalsSpreadsArr.length + " spread(s)";
            } catch (e) {
                originalsStatus = "ERROR: " + e.message + " (line " + e.line + ")";
            }
        } else {
            originalsStatus = "SKIPPED — falta: " + missingTpl.join("; ");
    ...
    // Remove ORIGINALS template pages (v1.16.0). Always — even for a no-annex
    // book — so a master carrying the templates never prints them. Removal is by
    // reference (captured in STEP 2, before any duplication), whole parent
    // spreads at once, deduped by spread id in case pages share a spread.
    // Nothing else may live on those spreads. Done before the TOC so the
    // deletion is accounted for in pagination, same as the recipe templates.
    var origTplPages = [originalsTitleTplPage, originals1upTplPage, originals2upTplPage];
    var origTplSpreads = [];
    for (var ot = 0; ot < origTplPages.length; ot++) {
        if (!origTplPages[ot]) continue;
        try {
            var spTpl = origTplPages[ot].parent;
            var dupSp = false;
            for (var od = 0; od < origTplSpreads.length; od++) {
                if (origTplSpreads[od].id === spTpl.id) { dupSp = true; break; }
            }
            if (!dupSp) origTplSpreads.push(spTpl);
        } catch (e) {}
    }
    for (var orm = 0; orm < origTplSpreads.length; orm++) {
        try {
            removeSpread(origTplSpreads[orm]);
            $.writeln("Removed originals template spread");
        } catch (e) {
            $.writeln("Error removing originals template spread: " + e.message);
        }
    }

    // ── STEP 10.5: Remove buffer blank page (by reference, v1.18.0) ──
    // Reason: the master needs a blank page at the end for AT_END duplication,
    // but the final book should not have it. Removal is strictly by the
    // reference captured in STEP 2 — a "last page looks blank" heuristic would
    // also match the orange colophon (a color rectangle with no text/images).
    // In the Generalv2 master the buffer shares the [2up|blank] template
    // spread, so STEP 10 usually removes it already; then nothing happens here.
    var blankRemoved = false;
    try {
        if (bufferBlankPage && bufferBlankPage.isValid) {
            blankRemoved = removePageIfBlank(bufferBlankPage);
            $.writeln(blankRemoved ? "Buffer blank page removed (by reference)" : "Buffer page kept (has content)");
        } else {
            $.writeln("Buffer blank page already gone (removed with template spread)");
        }
    } catch (eBuf) {
        $.writeln("Buffer blank removal error: " + eBuf.message);
    }
    ...
    // ── REPORT ──
    var msg = "LIBRO GENERADO (v1.18.0)\n\n";
    ...
    // Reason: keep the alert readable — full paths/URLs go to the console log
    // only; the alert says Placed or shows the failure detail.
    msg += "\nQR image: " + (qrImagePlaced ? "Placed" : QR_STATUS);
    msg += "\nRepurchase QR: " + (repurchaseQRPlaced ? "Placed" : REPURCHASE_QR_STATUS);
    $.writeln("QR: " + QR_STATUS + " | Repurchase: " + REPURCHASE_QR_STATUS);
...
// ============================================
// SIGNATURE PLACEMENT (v21)
// Reason: coloca la firma manuscrita del invitado (PNG transparente) dentro del marco
// {{SIGNATURE}} del template. Modelado en placeImageInSpread pero: (a) fit PROPORTIONALLY
// + CENTER_CONTENT para mostrar la firma COMPLETA sin recortar (como los Originals), y
// (b) SIN sendToBack (la firma va encima). Totalmente opcional y a prueba de errores:
...
// Finds the PAGE that carries a given item label (used to locate the three
// Originals template pages before any duplication). Returns the Page or null.
function findTemplatePageByItemLabel(doc, label) {
    var item = findItemByLabel(doc, label);
    if (!item) return null;
    try { return item.parentPage; } catch (e) { return null; }
}

function placeOriginalInFrame(page, frameLabel, localPath, basePath) {
    var frame = findItemOnPageByLabel(page, frameLabel);
    if (!frame) {
        $.writeln("  Original frame not found: " + frameLabel);
        return false;
    }
...
// Fills one inserted Originals page from a plan entry: 2 images -> TOP/BOTTOM
// frames (position order), 1 image -> the single {{ORIGINAL_IMAGE}} frame.
// One shared caption per page ("{recipe} shared by {guest}").
function fillOriginalsPage(page, planItem, basePath) {
    if (planItem.images.length === 2) {
        placeOriginalInFrame(page, CONFIG.originalImageTopLabel, planItem.images[0], basePath);
        placeOriginalInFrame(page, CONFIG.originalImageBottomLabel, planItem.images[1], basePath);
    } else {
        placeOriginalInFrame(page, CONFIG.originalImageLabel, planItem.images[0], basePath);
    }
    setCaptionOnPage(page, planItem.caption);
}
...
// Removes every top-level item from a page (master items untouched). Used to
// turn a shell page into a different layout (or into a true blank filler).
...
// Copies every top-level item from a template page onto a target page, keeping
// each item's position RELATIVE TO ITS PAGE. When source and target sit on
// OPPOSITE spread sides, the X position is MIRRORED (outer margin stays outer)
// because the master uses mirrored inner/outer margins — centered items are
// unaffected. Uses spread-origin ruler math and corrects from the duplicate's
// actual landing position, so it works no matter where duplicate(toPage) drops
// the copy.
function copyPageItemsTo(srcPage, dstPage) {
    var doc = app.activeDocument;
    var vp = doc.viewPreferences;
    var oldOrigin = vp.rulerOrigin;
    var oldZero = doc.zeroPoint;
    vp.rulerOrigin = RulerOrigin.SPREAD_ORIGIN;
    doc.zeroPoint = [0, 0];
    try {
        var srcPageB = srcPage.bounds;   // [y1, x1, y2, x2]
        var dstPageB = dstPage.bounds;
        var pageW = dstPageB[3] - dstPageB[1];
        var mirror = false;
        try { mirror = srcPage.side !== dstPage.side; } catch (eSide) {}
        var items = srcPage.pageItems.everyItem().getElements();
        for (var i = 0; i < items.length; i++) {
            var gbSrc = items[i].geometricBounds;
            var relY = gbSrc[0] - srcPageB[0];
            var relX = gbSrc[1] - srcPageB[1];
            var itemW = gbSrc[3] - gbSrc[1];
            var targetRelX = mirror ? (pageW - relX - itemW) : relX;
            var dup = items[i].duplicate(dstPage);
            var gbNow = dup.geometricBounds;
            dup.move(undefined, [
                (dstPageB[1] + targetRelX) - gbNow[1],
                (dstPageB[0] + relY) - gbNow[0]
            ]);
        }
    } finally {
        vp.rulerOrigin = oldOrigin;
        doc.zeroPoint = oldZero;
    }
}

// Removes a specific page if it has no real content (text or placed graphics).
// Used ONLY for the captured buffer blank — never for "whatever page is last".
...
// Builds the Originals section (v1.17.0) as whole SPREADS: duplicates the
// [title|1up] shell spread AT_END once per page pair, rebuilds slot layouts by
// copying template-page items, fills images/captions, then moves the spreads
// right after the anchor spread. The first page created for each recipe is
// recorded for the "See Originals Page {N}" cross-ref pass.
function generateOriginalsSpreads(titleTplPage, tpl1upPage, tpl2upPage, plan, basePath, anchorSpread, originalsSpreads, recipeFirstOrigPage, contentParent) {
    // Slot list: slot 0 = title page, then one slot per plan entry, then a blank
    // slot when the total is odd — slots pair into 2-page spreads, so the
    // section always ends on an even page count and never flips the side of
    // anything after it.
    var slots = [{title: true}];
    for (var i = 0; i < plan.length; i++) slots.push(plan[i]);
    if (slots.length % 2 === 1) {
        slots.push({blank: true});
        $.writeln("  Originals: parity blank slot added");
    }

    // The [title|1up] template spread is the shell for EVERY section spread:
    // left page carries the title items, right page the 1-up items. Each slot
    // that needs something else gets its page cleared and rebuilt from the
    // matching template page.
    var shellSrc = titleTplPage.parent;

    for (var s = 0; s < slots.length; s += 2) {
        var shell = shellSrc.duplicate(LocationOptions.AT_END);
        originalsSpreads.push(shell);
        for (var pIdx = 0; pIdx < 2; pIdx++) {
            var slot = slots[s + pIdx];
            var page = shell.pages[pIdx];
            if (slot.title) continue;                       // opener title page: keep as designed

            // Content/blank pages take the 1-up template's parent: running
            // footers and folios come from the parent, correct per page side.
            if (contentParent) {
                try { page.appliedMaster = contentParent; } catch (ePm) {}
            }
            if (slot.blank) { clearPageItems(page); continue; }

            var is2up = slot.images.length === 2;
            if (is2up) {
                clearPageItems(page);
                copyPageItemsTo(tpl2upPage, page);
            } else if (pIdx === 0) {
                // 1-up content on a LEFT page: the shell's left page carries
                // title items — clear and rebuild from the 1-up template.
                clearPageItems(page);
                copyPageItemsTo(tpl1upPage, page);
            }
            // (1-up on a RIGHT page: the shell already carries the 1-up items.)

            fillOriginalsPage(page, slot, basePath);
            if (!recipeFirstOrigPage[slot.recipeId]) {
                recipeFirstOrigPage[slot.recipeId] = page;
            }
        }
    }

    // Move the section into place: first spread right AFTER the anchor (last
    // Objects-that-stay spread), the rest chained behind it — the same spread
    // primitives STEP 9 uses for recipes.
    var prevSpread = anchorSpread;
    for (var m = 0; m < originalsSpreads.length; m++) {
        originalsSpreads[m].move(LocationOptions.AFTER, prevSpread);
        prevSpread = originalsSpreads[m];
    }
    $.writeln("  Originals: " + originalsSpreads.length + " spread(s) moved after anchor");
}
