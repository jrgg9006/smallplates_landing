// scripts/indesign/fetch-book_v4.js
...
// v3 (jul 2026): baja la firma por receta (guest_recipes.signature_url) a
// image_assets/{group}/signatures/{recipeId}.png y agrega signature_image_path.
// v4 (ago 2026): HERENCIA DE FIRMA. Si una receta no tiene firma propia
// (guest_recipes.signature_url null), hereda la firma canónica del invitado
// (guests.signature_url). Así, si el guest firma DESPUÉS en otra receta, sus
// recetas viejas sin firma también salen firmadas. El snapshot por-receta gana
// cuando existe (una receta firmada con A sigue imprimiéndose con A). No muta la
// DB: la herencia ocurre solo al armar el libro.
...
// TO RUN: node scripts/indesign/fetch-book_v4.js <GROUP_ID>
...
// Uso: node scripts/indesign/fetch-book_v4.js <GROUP_ID>
// ============================================
...
if (!process.argv[2]) {
  ...
  console.log('   Uso: node scripts/indesign/fetch-book_v4.js <GROUP_ID>');
  console.log('');
}
// ============================================
...
async function fetchBook() {
  console.log('');
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  📖 SMALL PLATES — Book Data Fetcher v4 (Signature inherit)');
  console.log('═══════════════════════════════════════════════════════════');
  console.log('');
  ...
  let signaturesDownloaded = 0;
  let signaturesInherited = 0;
  ...
  for (const recipe of recipes) {
    ...
    // Firma del invitado (opcional, PNG transparente). v4: la firma efectiva es la
    // propia de la receta (snapshot) y, si no existe, la canónica del invitado
    // (guests.signature_url). Reason: así una receta que el guest dejó sin firmar
    // hereda la firma que puso después en otra receta; el snapshot gana cuando existe
    // (recetas firmadas con firmas distintas se respetan). Solo se agrega la clave
    // signature_image_path cuando hay firma efectiva, para no romper la cero-regresión
    // en libros sin firma. Descarga tolerante a fallos: si falla, se omite y sigue.
    const effectiveSignatureUrl = recipe.signature_url || (guest && guest.signature_url) || null;
    const signatureInherited = !recipe.signature_url && Boolean(guest && guest.signature_url);

    if (effectiveSignatureUrl) {
      try {
        const signaturesDir = path.join(imagesDir, 'signatures');
        if (!fs.existsSync(signaturesDir)) {
          fs.mkdirSync(signaturesDir, { recursive: true });
        }
        const signatureFileName = `${recipe.id}.png`;
        const signatureDest = path.join(signaturesDir, signatureFileName);
        process.stdout.write(`   ✍️  firma${signatureInherited ? ' (heredada)' : ''} ${recipe.recipe_name}... `);
        await downloadImage(effectiveSignatureUrl, signatureDest);
        transformed.signature_image_path = `image_assets/${GROUP_ID}/signatures/${signatureFileName}`;
        if (signatureInherited) signaturesInherited++;
        else signaturesDownloaded++;
        console.log('✅');
      } catch (err) {
        signaturesFailed++;
        console.log(`❌ Error: ${err.message}`);
  ...
  if (signaturesDownloaded > 0 || signaturesInherited > 0 || signaturesFailed > 0) {
    const inheritedNote = signaturesInherited > 0 ? `, ${signaturesInherited} heredadas` : '';
    const failedNote = signaturesFailed > 0 ? `, ${signaturesFailed} fallidas` : '';
    console.log(`  ✍️  Firmas: ${signaturesDownloaded} propias${inheritedNote}${failedNote}`);
  }
  console.log('');
